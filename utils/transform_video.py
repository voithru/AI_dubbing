...
def merge_video_audio(video_path, audio_path, output_folder_path):
    ...
    origin_background_audio = Path(f'/dubbing/output/raw/{video_name}_0.wav')
    dubbed_muted_audio      = Path(f'/dubbing/output/raw/{video_name}_5.wav')
    ...
    subprocess.run([
            "ffmpeg", 
            "-ss", "5",                      # 시작 시간을 5초로 설정 (5초 이후의 음성 추출)
